...
description: Continuous attack-surface monitoring for first-mover advantage on bug-bounty programs — passive subdomain enumeration, certificate-transparency polling/streaming, httpx liveness diffing, JS bundle diffing, cloud/bucket discovery and verified secret-leak watch on systemd timers, alerting on DIFF ONLY. Use when the operator says "monitor new subdomains", "watch for new assets", "cert transparency", "certstream", "diff the JS bundles", "diff de bundles JS", "set up recon cron", "monitorear nuevos subdominios", "vigilar nueva superficie", "cron de recon", "alertame cuando aparezca un host nuevo".
