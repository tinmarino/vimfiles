...
description: 'Writes Python scripts that fan out many HTTP requests through a CONTINUOUS bounded worker pool (always N workers busy, no per-batch fork-join) and rotate the source IP via AWS API Gateway (requests-ip-rotator). Use for mass-enumeration / IDOR / fuzzing / bulk-fetch against one endpoint with --workers, --rotate, --resume, and a session.yaml refreshed by a keepalive sidecar when auth is short-lived. Composes with the python-writer skill for style.'
source: own-tooling
license: MIT
