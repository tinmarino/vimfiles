---
name: vuln-reporter
description: "Redacta hallazgos de pentest en español neutro chileno con el formato Markdown de CyScope (Title, CVSS, Sistemas afectados, Descripción, Impacto, Contramedidas, Referencias, Solicitud, Parámetro vulnerable, Carga, Prueba de concepto + tablas Adjuntos/Tiempos). Activar cuando un consultor redacte, convierta o revise un hallazgo técnico de auditoría en español para CyScope, incluyendo exportaciones JSON (`out/json`), Dreamlab `InformeCollection` o archivos `Template*.md`."
source: mine
license: MIT
...
- **Capturas anotadas navegador+Burp**: cuando el paso amerite una captura del navegador (recuadro azul sobre el botón/URL, flechas), NO la dibujes a mano: usa la skill `browser-burp-evidence`, que conduce el navegador proxificado por Burp, anota el PNG con `bin/annotate.py` (recuadro azul + flecha + etiqueta) y emite el paso ya con la frase "En el proxy de Burp Suite se capturó la solicitud HTTP y su respuesta...", el bloque ` ```ruby ` de la solicitud/respuesta y la imagen bajo `res/`. Cita el PNG resultante como cualquier otra imagen (solo si existe en disco).
