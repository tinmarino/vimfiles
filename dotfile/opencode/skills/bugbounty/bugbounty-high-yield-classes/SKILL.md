...
description: Ranked dollars-per-hour playbook of bug-bounty vulnerability classes (IDOR/BOLA, broken auth, business logic and races, SSRF, GraphQL authz, mass assignment, JWT, upload-to-RCE, subdomain takeover) with a per-class hunting checklist, keep-digging signal, and the informative-closure to pre-empt. Triggers on "what should I hunt next", "which bug class pays best", "pick the next attack surface", "que busco ahora", "que clase de bug paga mas", "cual es el mejor uso de mi tiempo en este programa". For escalating one specific finding a tier, use bugbounty-impact-escalation.
