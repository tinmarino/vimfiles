...
description: Turn a Low/Medium bug-bounty finding into a High/Critical payout by chaining primitives and proving business impact — the escalation ladder per class, safe maximum-impact PoCs, cross-account and privilege-boundary proofs, and how to document a chain so triage scores the chain instead of its weakest link. Use when the operator says "esto vale poco, cómo lo subo", "cómo escalo este hallazgo", "puedo encadenar esto", "me lo van a cerrar como informativo", "prueba el cruce de cuentas", "demuestra el impacto real", "how do I escalate this", "can I chain these two bugs", "this is only medium", "prove business impact", "argue critical instead of high", "before I submit this low", or whenever a confirmed primitive (info leak, weak ID, exported component, blind SSRF, mass assignment) is about to be submitted as-is.
license: MIT
